...
class Bus extends CI_Controller {
	...
	public function tambahbus(){
		...
		$this->session->set_flashdata('message', 'swal("Berhasil", "Data Travel Di Simpan", "success");');
		redirect('backend/bus');
	}
	
	public function editbus($kd_bus){
		$kode = $this->getkod_model->get_kodbus();
		$data = array(
			...
			'plat_bus'		 => $this->input->post("plat_bus"),
			'kapasitas_bus'	 => $this->input->post("seat"),
			'status_bus'	 => $this->input->post("status_bus"),
			 );
		// die(print_r($data));
		$this->db->update('tbl_bus', $data, $id);
		$this->session->set_flashdata('message', 'swal("Update Berhasil", "Data Travel Di Perbarui", "success");');
		redirect('backend/bus');
	}
}	
